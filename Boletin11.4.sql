USE LeoMetroV2
SET Dateformat 'ymd'
--0. La dimisión de Esperanza Aguirre ha causado tal conmoción entre los directivos de LeoMetro que han decidido conceder una amnistía a todos 
--los pasajeros que tengan un saldo negativo en sus tarjetas.
--Crea un procedimiento que racargue la cantidad necesaria para dejar a 0 el saldo de las tarjetas que tengan un saldo negativo y hayan 
...
DECLARE @ID_Tarjeta int = 3
DECLARE @Importe smallmoney = 0.20		
...
BEGIN
	...
	-- Creo una nueva fecha con el año y mes anterior pero el ultimo dia del mes(para ello utilizamos el EOMONTH que coge el ultimo dia del mes de la fecha)
	SET @finMes = DATEFROMPARTS(YEAR(@mesAnterior), MONTH(@mesAnterior),DAY(EOMONTH(@mesAnterior)))

	-- Insertamos los pasajeros que hayan gastado más de 30 euros y no hayan usado más de 10 veces alguna estación de las zonas 3 o 4 
	...
	-- Insertamos los pasajero que hayan usado más de 10 veces alguna estación de las zonas 3 o 4 y no hayan gastado mas de 30 euros en el 
...
GO
CREATE  PROCEDURE RecargarCantidadPositiva 
AS
BEGIN
	BEGIN TRANSACTION
