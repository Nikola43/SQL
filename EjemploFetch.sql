...
Al a�adir un bucle hay que asegurarse de recorrer inserted y de a�adir un 
indicador para salir del bucle cuando una fila incumpla la condici�n
*/
...
BEGIN
	...
	IF EXISTS (SELECT * FROM inserted AS I
		...
		Where C1.Modalidad <> C2.Modalidad) -- La condici�n del If pasa al WHERE
		BEGIN
		Raiserror ('Los contrincantes deben ser de la misma modalidad',6,1)
		ROLLBACK
		END --If
...
-- Versi�n todas las filas arrastrandonos penosamente a recorrer la tabla inserted
CREATE TRIGGER MismaModalidad ON LM_Partidos AFTER INSERT AS
BEGIN
	DECLARE @Contrincante1 CHAR (6), @Contrincante2 Char(6), @Edicion SmallInt
	DECLARE @Modalidad1 CHAR(2), @Modalidad2 CHAR(2)
	...
	While @Cont <= @NumFilas AND @Seguimos = 1 -- A�adimos el indicador
	Begin
		...
		IF @Modalidad1 <> @Modalidad2
			...
			ROLLBACK
			END
		-- Actualizamos la condici�n del bucle 
		SET @cont += 1
	End -- While
END --TRIGGER MismaModalidad
GO
...
-- Versi�n todas las filas arrastrandonos penosamente a recorrer la tabla inserted con un cursor
CREATE TRIGGER MismaModalidad ON LM_Partidos AFTER INSERT AS
BEGIN
	DECLARE @Contrincante1 CHAR (6), @Contrincante2 Char(6), @Edicion SmallInt
	DECLARE @Modalidad1 CHAR(2), @Modalidad2 CHAR(2)
	--Esto es lo que var�a
	DECLARE @Numero SmallInt = -1, @Seguimos Bit = 1
	-- As� se declara el cursor. Tomamos solo las columnas que vayamos a necesitar
